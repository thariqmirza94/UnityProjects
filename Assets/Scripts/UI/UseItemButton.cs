...
public class UseItemButton : MonoBehaviour
{
    ...
    public void UseItem()
    {
        ...
        if (_item is HealingPill healingPill)
        {
            healingPill.GetPlayer();
            healingPill.Heal();
            _inventory.RemoveItem(_item.ID);
            _inventoryUi.RefreshInventoryUI();
            itemAction = _item.ItemActionText();
        }
        else if (_item is Axe axe)
        {
            axe.GetMonster(_monster);
            axe.Attack();
            itemAction = _item.ItemActionText();
        }
        else if (_item is Cleaver cleaver)
        {
            cleaver.GetMonster(_monster);
            cleaver.Attack();
            itemAction = _item.ItemActionText();
        }
        else if (_item is Mace mace)
        {
            mace.GetMonster(_monster);
            mace.Attack();
            itemAction = _item.ItemActionText();
        }
        else
